...
from src.utils.config import PKL_FILE_PATH, SPARSE_RETRIEVAL_CONFIG, RESULTS_DIR
import json
...
def main():
    ...
    print("=" * 80)
    print("Sparse Retrieval (희소 검색) 테스트")
    print("=" * 80)
    ...
    for i, test_item in enumerate(test_queries, 1):
        ...
        results = retriever.search(test_item['query'], top_k=5)
        retriever.print_results(test_item['query'], results)
    ...
    output_path = RESULTS_DIR / "sparse_retrieval_results.json"
    
    output_data = {
        'timestamp': datetime.now().isoformat(),
        'method': 'Sparse Retrieval (BM25 + Korean Tokenizer)',
        'tokenizer': retriever.tokenizer_type if hasattr(retriever, 'tokenizer_type') else 'unknown',
